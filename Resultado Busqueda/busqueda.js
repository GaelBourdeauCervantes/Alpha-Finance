...
function hacerGraficaROIC(
	...
	const totalDuration = duracionAnimacion;
	const delayBetweenPoints = totalDuration / data.length;
	const previousY = (ctxROIC) =>
	ctxROIC.index === 0
			? ctxROIC.chart.scales.y.getPixelForValue(100)
			: ctxROIC.chart.getDatasetMeta(ctxROIC.datasetIndex).data[ctxROIC.index - 1].getProps(['y'], true).y;
	...
	// Hacer Difuminado de color
	gradianteROIC = ctxROIC.createLinearGradient(0, 0, 0, alturaContexto);
	gradianteROIC.addColorStop(0, 'rgb(250, 196, 0)');
	gradianteROIC.addColorStop(1, fondo);

	// Preparar info
	colorFondoROIC = gradianteROIC;
	colorLineaROIC = 'rgb(250, 196, 0)';
			...
			datasets: [
				{
					...
					pointHoverRadius: 8,
					pointHoverBackgroundColor: '#fff',
					borderColor: colorLineaROIC,
					tension: 0.5,
					fill: true,
					backgroundColor: colorFondoROIC,
					borderWidth: anchoLinea,
				},
			],
		},
				...
				tooltip: {
					...
					callbacks: {
						label: function(tooltipItems, data) { 
							...
							return 'Roic: ' + arondir(roicEnPosicion*100) + '%';
						}
					}
				}
			},
...
function trabajarData(data) {
	...
	function ROIC(
		...
		// console.log(millonesBillonesTrillones(nopat))
		// console.log(millonesBillonesTrillones(investedCapital))
		// console.log(porcentaje(roic))
	...
	hacerGraficaROIC('graficaROIC',arrayRoic.reverse(),años.reverse(),2,300,0,ticker)
	// Hacer Tabla ROIC
	// Invertir arrays
	años.reverse()
	arrayRoic.reverse()
	...
	// Grwoth Rank
	documentoGrwothRankTituloTabla.innerHTML = 'Grwoth Rank ' + numeroAños + ' years'
...
urlMaxW =
	...
	'&to=' +
	fechaHoy +
	'&period=w&fmt=json&api_token=OeAFFmMliFG5orCUuwAKQ8l4WWFQ67YX';
