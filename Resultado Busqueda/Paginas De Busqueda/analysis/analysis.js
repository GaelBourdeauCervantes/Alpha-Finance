...
function trabajarData(data) { 
	...
	if(tomarValorLocalStorage(ticker + ': expectedReturn') == 'undefined') {
		expectedReturn = document.getElementById('expectedReturn').value = '11%'
	} else {
	...
	tresuryYieldValue.then(value => {
		...
		// Calcular CAPM
		CAPM = verPorcentaje(riskFreeRateElement.value) + data['Technicals']['Beta'] * (expectedReturn - riskFreeRate)
		requieredReturnElement.innerHTML = porcentaje(CAPM)
		if(tomarValorLocalStorage(ticker + ': requieredReturn') == 'undefined') {
			inputRiquiredRateOfReturn.value = porcentaje(CAPM)
		} else {
	...
	inputs.forEach(input => {
		input.addEventListener('change', () => {
			...
			requieredReturnElement.textContent = porcentaje(CAPM)
			requieredReturn = verPorcentaje(inputRiquiredRateOfReturn.value)
...
// Esperar 3 segundos después de cargar la página
	...
	setTimeout(ejecutarDespuesDeCargar, 1000); // 3000 milisegundos = 3 segundos
});
