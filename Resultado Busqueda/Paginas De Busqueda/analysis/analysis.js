...
function trabajarData(data) { 
	...
	tresuryYieldValue.then(value => {
		...
		for(i=0;i<arrayFechasBien.length;i++) {
			...
			if(i < añosAtras) {
				...
				crearYAgregarElementoInput(trTotalRevenue,millonesBillonesTrillones(arrayTotalRevenue[i]),'ponerMargen')
			}
		}
		...
		// Hacer Proyeccion----------
		// Calcular previo CAGR
		CAPEXCAGR = (arrayCAPEX[añosAtras-1] / arrayCAPEX[0])**(1/añosAtras) - 1
		//Poner OCF Margen promedio 
		//Poner como info
		avrage5YearsCAPEXCAGRTitle = document.getElementById('avrage5YearsCAPEXCAGRTitle')
		avrage5YearsCAPEXCAGRTitle.innerText = 'Average ' + añosAtras + ' years CAGR'
		avrage5YearsCAPEXCAGRDOC = document.getElementById('avrage5YearsCAPEXCAGR')
		avrage5YearsCAPEXCAGRDOC.innerText = porcentaje(CAPEXCAGR)
		// Poner en input
		estimatedNext5YearsCAPEXCAGRTitle = document.getElementById('estimatedNext5YearsCAPEXCAGRTitle')
		estimatedNext5YearsCAPEXCAGRTitle.innerText = 'Next ' + añosAproyectar + ' years CAGR'
		nextCAPEXCAGRInput = document.getElementById('nextCAPEXCAGRInput')
		// Ver si ya se cambio el estimado, si el estimado es diferente que el Margen propuesto entonces poner el que se cambio
		if(tomarValorLocalStorage(ticker + ': estimatedCAPEXCost') == 'undefined') {
			nextCAPEXCAGRInput.value = porcentaje(CAPEXCAGR)
		} else {
			nextCAPEXCAGRInput.value = porcentaje(tomarValorLocalStorage(ticker + ': estimatedCAPEXCost'))
		}
		// Hacer proximos 5 anos CAPEX Y agregar a array
		nextCAPEXCAGRInput = verPorcentaje(nextCAPEXCAGRInput.value)
		for(i=0;i<añosAproyectar;i++) {
			CAPEX = CAPEX * (1 + nextCAPEXCAGRInput)
			arrayCAPEX.push(CAPEX)
		}
		// Crear Grafica
		...
		for(i=0;i<arrayFechasBien.length;i++) {
			...
			if(isNaN(arrayFCFDiscounted[i])) {
				...
				crearYAgregarElementoInput(trFreeCashFlow,millonesBillonesTrillones(arrayFCF[i]))
			}
		}
	...
	inputs.forEach(input => {
		input.addEventListener('change', () => {
			...
			guardarValorLocalStorage(ticker + ': estimatedCAPEXCost',nextCAPEXCAGRInput)
			// Quitar Elementos de la tabla
...
// Function crear y agregar elemento texto
...
// Function crear y agregar elemento en texto en forma de input
function crearYAgregarElementoInput(elementoPadre,textoDeElementoCreacion, margen) {
	nuevoElementoTd = document.createElement('td')
	nuevoElementoInput = document.createElement('input')
	nuevoElementoInput.type = 'text'
	nuevoElementoInput.value = textoDeElementoCreacion
	if(margen == 'ponerMargen') {
		nuevoElementoInput.style.marginTop = '9px';
		nuevoElementoInput.style.marginBottom = '10px';
	} 
	nuevoElementoTd.appendChild(nuevoElementoInput)
	elementoPadre.appendChild(nuevoElementoTd)
	return nuevoElementoTd
}
