...
async function asyncTrabajarDataIndicadores(url) {
	// Storing response
	const response = await fetch(url);

	// Storing data in form of JSON
	let data = await response.json();
	trabajarDataIndicadores(data)
}

// Definir Function tomar trabajar data
function trabajarDataIndicadores(data) {
	...
	arrayAnualClose.then(arrayAnualClose => {
		...
		for(i = 0; i < largoLoop - 1; i++) {
			...
			// Calcular ROE
			roe = netIncome / balanceSheet['totalStockholderEquity']

			// Calcular ROA
			roa = netIncome / totalAssets

			// Calcular Quick Ratio
			quickRatio = ( currentAssets - balanceSheet['inventory'] ) / currentLiabilities

			// Calcular EBIT / Intrest
			coverageRatio = incomeStatement['ebit'] / incomeStatement['interestExpense']
			...
			if(numero5Atras != 1) {
				...
				arrayEps.push(eps)
				arrayRoic.push(roic)
				arrayRoe.push(roe * 100)
				arrayRoa.push(roa * 100)
				arrayQuickRatio.push(quickRatio)
				arrayCoverageRatio.push(coverageRatio)
				arrayMinimoQuickRatio.push(1)
		...
		arrayRoe.reverse()
		arrayRoa.reverse()
		arrayQuickRatio.reverse()
		arrayPuntuacion.reverse()
		arrayCoverageRatio.reverse()
		...
		function hacerTTM(closePriceAperturaCitio){
			...
			// Agrgar ROA TTM
			arrayRoa.push(roaTTM * 100) 

			// Agregar ROE TTM
			roeTTM = netIncomeTTM / balanceSheetMRQ['totalStockholderEquity']
			arrayRoe.push(roeTTM * 100)

			// Agregar Quik Ratio TTM
			quickRatioTTM = ( currentAssetsMRQ -  balanceSheetMRQ['inventory']) / currentLiabilitiesMRQ
			arrayQuickRatio.push(quickRatioTTM)
			arrayMinimoQuickRatio.push(1)

			// Calcular EBIT / Intrest
			yearlyData = data['Income_Statement']['yearly']
			llaves = Object.keys(yearlyData)
			coverageRatio = calcularTTM(data,'Income_Statement','ebit') / calcularTTM(data,'Income_Statement','interestExpense')
			arrayCoverageRatio.push(coverageRatio)
			...
			colorPositivo = '#94c59b'
			colorMedio = '#ffe81a'
			colorNegativo = '#e29c9c'
			...
			if(roaTTM > 0) {
				...
				currentROA.style.color = colorNegativo
			}
			// CFRO
			...
			if(zScoreTTM <= 1.8) {
				...
				opinionZscore.style.color = '#e76f51'
				...
				opinionZscore.style.color = '#6c757d'
				...
				opinionZscore.style.color = '#2a9d8f'
			...
			if(mScoreTTM <= -2.22) {
				...
				opinionMScore.style.color = '#2a9d8f'
				...
				opinionMScore.style.color = '#6c757d'
			...
			// Hacer grafica ROE & ROA
			graficaROARE('graficaROAROE',arrayRoa,arrayRoe,labelsGrafica)
			// Hacer Grafica Quick Ratio
			graficaRatioMinimo('graficaQuickRatio', 'Quick Ratio', arrayQuickRatio, 1, labelsGrafica, '#2E8B57')
			// Hacer Grafuca Coverage ratio
			graficaRatioMinimo('graficaCoverageRatio', 'Coverage Ratio', arrayCoverageRatio, 1.5, labelsGrafica, '#1F618D')
		}
...
asyncTrabajarDataIndicadores('https://eodhistoricaldata.com/api/fundamentals/'+ ticker +'?&api_token='+apiToken)
			...
			scales: {
				...
				ticks: {
					callback: function(value) {
						return value + ' / 100'; // Añade el símbolo de porcentaje a las etiquetas del eje y
					}
				},
					max: max,
					min: min,
...
// Grafica ROA ROE
function graficaROARE(idGrafica, arrayRoa, arrayRoe, labels) {
	var ctx = document.getElementById(idGrafica).getContext('2d');
	var myChart = new Chart(ctx, {
		type: 'bar',
		labels: labels,
		...
		options: {
				...
				tooltip: {
					...
					callbacks: {
						label: function(context) {
							return context.dataset.label + ': ' + arondir(context.parsed.y)  + '%';
						}
					}
				}
			},
			scales: {
				ROA: {
				  ...
				  ticks: {
						callback: function(value) {
							return value + '%'; // Añade el símbolo de porcentaje a las etiquetas del eje y
						}
					}
				},
				ROE: {
...
// Grafica Quick Ratio
function graficaRatioMinimo(idGrafica, nombreMultiplo,  arrayMultiplo, valorMinimo, labels, color) {
	arrayValorMinimo = []
	for(i = 0; i < arrayMultiplo.length; i++) {
		arrayValorMinimo.push(valorMinimo)
	}
		...
		options: {
			plugins: {
				...
				tooltip: {
					...
					callbacks: {
						label: function(context) {
							return context.dataset.label + ': ' + arondir(context.parsed.y)  + ' X';
						}
					}
				}
			},
...
function colorPuntuacion(puntuacion) {
	if (puntuacion <= 10) {
		return 'rgb(156, 163, 175)'; // #9CA3AF gris claro
	} else if (puntuacion <= 30) {
		return 'rgb(107, 114, 128)'; // #6B7280 gris medio
	} else if (puntuacion <= 40) {
		return 'rgb(84, 140, 146)';  // #548C92 azul Alpha
	} else if (puntuacion <= 50) {
		return 'rgb(59, 125, 137)';  // #3B7D89 azul intermedio
	} else if (puntuacion < 60) {
		return 'rgb(50, 106, 117)';  // #326A75 azul más fuerte
	} else if (puntuacion < 70) {
		return 'rgb(43, 89, 99)';    // #2B5963 azul profundo
	} else {
		return 'rgb(31, 62, 71)';    // #1F3E47 azul muy oscuro
	}
}
