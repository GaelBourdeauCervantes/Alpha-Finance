...
function trabajarDataIndicadores(data) {
	...
	arrayAnualClose.then(arrayAnualClose => {
		...
		function hacerTTM(closePriceAperturaCitio){
			...
			graficaQuickRatio('graficaQuickRatio', arrayQuickRatio,arrayMinimoQuickRatio, labelsGrafica)
		}
...
function graficaQuickRatio(idGrafica, arrayQuickRatio, minimoQuickRatio, labels) {
	var ctx = document.getElementById(idGrafica).getContext('2d');
	var myChart = new Chart(ctx, {
		type: 'bar',
		labels: labels,
		data: {
			...
			datasets: [
			{
				label: 'Quick Ratio',
				type: 'line',
				data: arrayQuickRatio,
				backgroundColor: '#21DE32',
				borderColor: '#21DE32',
			},
			{
				label: 'Danger Leve;',
				type: 'line',
				data: minimoQuickRatio,
