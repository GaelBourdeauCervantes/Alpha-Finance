...
function tarabajarMiIndicador(data) {
	...
	arrayAnualClose.then(arrayAnualClose => {
		...
		function hacerTTM(closePriceAperturaCitio){
			...
			circuloScore = document.getElementById('circuloScore')
			if(fScoreTTM <= 3) {
				circuloScore.style.backgroundColor = colorNegativo
			} else if(fScoreTTM <= 6) {
				circuloScore.style.backgroundColor = colorMedio
			} else {
				circuloScore.style.backgroundColor = colorPositivo
			}
			// Gradiate 
